...
async function readDB(env2) {
  if (env2.ELVEN_KV) {
    const data = await env2.ELVEN_KV.get("elven_db", "json");
    if (data) {
      if (!data.users) data.users = [];
      return data;
    }
  } else if (env2.KV_REST_API_URL && env2.KV_REST_API_TOKEN) {
    try {
      const res = await fetch(`${env2.KV_REST_API_URL}/get/elven_db`, {
        headers: { Authorization: `Bearer ${env2.KV_REST_API_TOKEN}` }
      });
...
async function writeDB(env2, data) {
  if (env2.ELVEN_KV) {
    await env2.ELVEN_KV.put("elven_db", JSON.stringify(data));
  } else if (env2.KV_REST_API_URL && env2.KV_REST_API_TOKEN) {
    await fetch(`${env2.KV_REST_API_URL}/set/elven_db`, {
      method: "POST",
      headers: { Authorization: `Bearer ${env2.KV_REST_API_TOKEN}` },
...
// ../.wrangler/tmp/pages-nZRiI2/functionsRoutes-0.2693912874302773.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
